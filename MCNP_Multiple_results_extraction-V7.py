...
def find_tally_index(data):
    ''' This finds the final block of tally inputs based on the last appearance ...
        of the phrase " the 10 statistical checks..." and the following 1tally fluc...'''
    ...
    for i in range(len(data)):
        if data[i] == ' the 10 statistical checks are only for the tally fluctuation chart bin and do not apply to other tally bins.\n':
            stats.append(i)
        if data[i] == '1tally fluctuation charts                              \n':
            l_tals.append(i)
    
    for j in range(len(l_tals)):
        if l_tals[j] > stats[-1]:
            r_tals.append(l_tals[j])
            
    tal_start = r_tals[0]
    return tal_start
 
def print_results(dataframes, filelist):
    ...
    RESULT.to_excel(writer, "MCNP_Data", merge_cells=False)

    #Add conditional fomatting toggle
    add_conditional_formatting(writer)

    # Save new excel
    writer.save()   

def add_conditional_formatting(writer):
    
    # Select worksheet
    sheet = writer.sheets["MCNP_Data"]
    
    # Set book for formats
    ...
    sheet.conditional_format('C2:I999', {'type':     'formula',
                                      'criteria': '= $F2 > 0.05',
                                      'format':   format1})
    sheet.conditional_format('C2:I999', {'type':     'formula',
                                      'criteria': '= $G2 > 0.10',
...
#        i += 1
    ...
    # finds the final block of tally outputs
    n =  find_tally_index(datarray) + 1
    running =True
...
print_results(dataframes, filelist)
